...
class LoansController extends ResourceController
{
    ...
    public function create()
    {
        ...
        // ✅ Tambahkan validasi batas maksimal peminjaman 3 buku
        $existingLoansCount = $this->loanModel
            ->where('member_id', $member['id'])
            ->where('return_date', null)
            ->countAllResults();
    
        if ($existingLoansCount + count($bookSlugs) > 3) {
            session()->setFlashdata([
                'msg' => 'Peminjaman gagal. Anggota ini telah meminjam ' . $existingLoansCount . ' buku, dan maksimal hanya boleh meminjam total 3 buku.',
                'error' => true
            ]);
            return redirect()->to('admin/loans');
        }
        // ✅ End validasi batas peminjaman
...
{
    $memberId = $this->request->getPost('qrcode');
    ...
    return view('admin/transactions/form', ['member' => $member]);
}
