...
class MembersController extends ResourceController
{
    ...
    public function kirimEmail($uid)
    {
        $model = new MemberModel();
        $member = $model->where('uid', $uid)->first();
    
        if (! $member) {
            throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound('Member not found');
        }
    
        $qrCodePath = MEMBERS_QR_CODE_PATH . $member['qr_code'];
    
        if (!file_exists($qrCodePath)) {
            throw new \RuntimeException('QR Code tidak ditemukan.');
        }
    
        // Email setup
        $email = \Config\Services::email();
        $email->setTo($member['email']);
        $email->setSubject('QR Code Member Anda');
    
        // Gunakan CID
        $email->attach($qrCodePath, 'inline');
        $cid = basename($qrCodePath);
    
        $message = '
            <h3>Halo ' . esc($member['first_name']) . ',</h3>
            <p>Berikut adalah QR Code keanggotaan perpustakaan Anda:</p>
        ';
        ...
        if (!$email->send()) {
            ...
            echo 'Email berhasil dikirim ke ' . $member['email'];
        }
        '<p>Silakan tunjukkan QR Code ini saat berkunjung ke perpustakaan.</p>';
    }
...
// Fungsi helper (asumsi lokasinya)
if (!function_exists('deleteMembersQRCode')) {
    function deleteMembersQRCode($qrCodeFilename = null)
    {
        if ($qrCodeFilename && file_exists(MEMBERS_QR_CODE_PATH . $qrCodeFilename)) {
            unlink(MEMBERS_QR_CODE_PATH . $qrCodeFilename);
        }
    }
}

// Konstanta (asumsi lokasinya)
if (!defined('MEMBERS_QR_CODE_PATH')) {
    define('MEMBERS_QR_CODE_PATH', FCPATH . 'uploads/qr_codes/members/'); // Contoh path
}